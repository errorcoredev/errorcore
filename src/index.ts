...
const INSTANCE_GENERATION_KEY = Symbol.for('errorcore.sdk.instance.generation');
...
function setGlobalInstance(instance: SDKInstance | null): void {
  ...
  (globalThis as Record<symbol, number>)[INSTANCE_GENERATION_KEY] =
    ((globalThis as Record<symbol, number>)[INSTANCE_GENERATION_KEY] ?? 0) + 1;
}
...
export function getCaptureMode(): CaptureMode | null {
  const instance = getGlobalInstance();
  return instance === null ? null : instance.getCaptureMode();
}

export async function setCaptureMode(mode: CaptureMode): Promise<ModeSwitchResult> {
  const instance = getGlobalInstance();
  if (instance === null) {
    throw new Error('SDK is not initialized');
  }
  return instance.setCaptureMode(mode);
}
...
export { createSDK };
export { attachSupplementalLocals } from './capture/supplemental-locals';
...
/**
 * @internal
 * Monotonic singleton generation used by middleware to cache module instance
 * resolution without requiring this module on every request.
 */
export function getModuleInstanceGeneration(): number {
  return (globalThis as Record<symbol, number>)[INSTANCE_GENERATION_KEY] ?? 0;
}
