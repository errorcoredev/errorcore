        ...
        <div class="detail">
          ...
          \${io.length > 0 && html\`<div class="section">
            ...
            \${!collapsed.io && io.map(ev => html\`
              <div class="io-item">
                ...
                <span class="io-target">\${escapeHtml(ev.url || ev.target)}\${ev.dbMeta?.query ? ' \\u2014 ' + escapeHtml(String(ev.dbMeta.query)) : ''}</span>
          ...
          \${req && html\`<div class="section">
            ...
            \${!collapsed.req && html\`<dl class="kv-grid">
              ...
              \${req.headers && Object.entries(req.headers).map(([k,v]) => html\`<dt>\${escapeHtml(k)}</dt><dd>\${escapeHtml(String(v))}</dd>\`)}
            </dl>\`}
          </div>\`}
