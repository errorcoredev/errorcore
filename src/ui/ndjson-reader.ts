...
export class NdjsonReader {
  ...
  public getAll(options?: {
    ...
    if (options?.fingerprint) {
      entries = entries.filter((e) => e.fingerprint === options.fingerprint);
    }
  ...
  private parseLine(line: string): unknown | null {
    try {
      return this.parseParsed(JSON.parse(line), 0);
    } catch {
      return null;
    }
  }

  private parseParsed(parsed: unknown, depth: number): unknown | null {
    if (depth > 4) {
      return null;
    }
    ...
    try {
      ...
      if (
        ...
        return this.parseParsed(JSON.parse(plaintext), depth + 1);
      }
      ...
      if (
        ...
        return this.parseParsed(
          JSON.parse((parsed as { payload: string }).payload),
          depth + 1
        );
      }
      ...
      if (
        ...
        parsed !== null &&
        (parsed as { version?: unknown }).version === 1 &&
        (parsed as { kind?: unknown }).kind === 'payload' &&
        typeof (parsed as { payload?: unknown }).payload === 'string'
      ) {
        return this.parseParsed(
          JSON.parse((parsed as { payload: string }).payload),
