...
export function installFetchWrapper(deps: FetchWrapperDeps): FetchWrapperHandle {
  ...
  const patched: typeof globalThis.fetch = async function patchedFetch(
    ...
    const traceHeaders = context === undefined ? null : deps.als?.getTraceHeaders?.() ?? null;
    ...
    try {
      ...
      if (slot !== null) {
        ...
        mergePreparedRequestHeaders(slot, deps, prepared.requestHeaders);
    ...
    try {
      await deps.bodyCapture.captureUndiciResponseStream(
        ...
        (oldBytes, newBytes) => {
          deps.buffer.updatePayloadBytes(oldBytes, newBytes, slot);
        }
      );
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
