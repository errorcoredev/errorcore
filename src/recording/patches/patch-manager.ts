...
const SDK_WRAPPER_DEACTIVATE = Symbol('errorcore.sdkWrapperDeactivate');

interface OwnedWrapperInstallation {
  wrapper: Function;
  restore: () => void;
}

type WrappedTarget = Record<string | symbol, unknown> & {
  [OWNED_METHODS]?: Map<string, OwnedWrapperInstallation>;
};
...
type OwnedWrapperFunction = Function & {
  ...
  [SDK_WRAPPER_DEACTIVATE]?: () => void;
};

function getOwnedMethodStore(target: WrappedTarget): Map<string, OwnedWrapperInstallation> {
  ...
  return target[OWNED_METHODS] as Map<string, OwnedWrapperInstallation>;
}

function getOwnedWrapperMetadata(
  candidate: unknown,
  methodName: string
): { predecessor: Function; deactivate: () => void } | null {
  ...
  if (
    ...
    typeof wrapper[SDK_WRAPPER_DEACTIVATE] !== 'function'
  ) {
    return null;
  }
  ...
  return {
    ...
    deactivate: wrapper[SDK_WRAPPER_DEACTIVATE] as () => void
  };
}
...
function markOwnedWrapper(
  ...
  Object.defineProperty(ownedWrapper, SDK_WRAPPER_DEACTIVATE, {
    value: deactivate,
    configurable: false,
    enumerable: false,
    writable: false
...
export function installOwnedWrapper(
  ...
  const existing = getOwnedWrapperMetadata(current, methodName);
  existing?.deactivate();
  const predecessor = existing?.predecessor ?? current;
  let implementation: Function | null = wrapperFactory(predecessor);
  ...
  const gatedWrapper = function ownedRuntimeGate(this: unknown, ...args: unknown[]) {
    const activeImplementation = implementation;
    return Reflect.apply(activeImplementation ?? predecessor, this, args);
  };
  const ownedWrapper = markOwnedWrapper(
    gatedWrapper,
    methodName,
    predecessor,
    deactivate
...
export function wrapMethod(target: object, methodName: string, wrapper: Wrapper): void {
  ...
  const existing = store.get(methodName);
  ...
  store.set(methodName, installation);
}
...
export function unwrapMethod(target: object, methodName: string): void {
  ...
  const installation = store.get(methodName);
