...
import type { Policy } from './scrubber/types';
...
export interface RequestContext {
  ...
  /** Accepted inbound tracestate header before our vendor entry is stripped. */
...
export type CaptureMode = 'safe' | 'balanced' | 'forensic' | 'fast';

export interface CaptureRequestContextInput {
  method: string;
  url: string;
  headers?: Record<string, unknown>;
  statusCode?: number;
...
export interface CaptureErrorOptions {
  request?: CaptureRequestContextInput;
}

export type LocalVariablesMode = 'none' | 'shallow' | 'deep';

/**
 * Derived, read-only behavior switches computed from captureMode plus
 * explicit user overrides. Pipeline code branches on these — never on
 * captureMode string comparisons — so a mode is fully described by data.
 */
export interface CaptureCapabilities {
  /**
   * Buffer serialized payloads at capture and deliver via flush()/the
   * flush timer instead of dispatching the transport inside the request
   * window. True for the fast/safe chassis unless the user explicitly
   * re-enables worker assembly (worker-assembled packages dispatch
   * in-window by design).
   */
  deferredDelivery: boolean;
  /**
   * Materialize spooled request/response bodies into the package at
   * capture time. Follows the body-capture flags: when no bodies are
   * being captured there is nothing to materialize.
   */
  materializeBodies: boolean;
  /**
   * Synthesize an inbound http-server IO event when the timeline is empty
   * and the caller passed explicit request data.
   */
  syntheticInboundFallback: boolean;
  /** Run the standing event-loop lag measurement timer. */
  eventLoopLagMonitor: boolean;
}
...
export interface ModeState {
  ...
  localVariablesMode: LocalVariablesMode;
  capabilities: CaptureCapabilities;
  recorders: RecorderSettings;
  captureLocalVariables: boolean;
  localsGuard: {
    enabled: boolean;
    maxPausesPerSecond: number;
    maxPauseMsPerMinute: number;
  };
  ...
  maxLocalsObjectProperties: number;
}
...
export interface AdaptiveCaptureConfig {
  enabled?: boolean;
  base?: CaptureMode;
  escalated?: CaptureMode;
  deescalateAfterMs?: number;
  minDwellMs?: number;
  maxSwitchesPerHour?: number;
}

export type AdaptiveCapturePhase = 'inactive' | 'base' | 'escalated' | 'manual' | 'pinned';

export interface ResolvedAdaptiveCaptureConfig {
  enabled: boolean;
  base: CaptureMode;
  escalated: CaptureMode;
  deescalateAfterMs: number;
...
export interface AdaptiveCaptureHealth {
  active: boolean;
  phase: AdaptiveCapturePhase;
 ...
 * or drop events. See BACKPRESSURE.md for the full matrix - what each
...
export type InternalWarningCode =
  ...
  | 'EC_PACKAGE_ASSEMBLY_WORKER_FAILED'
  | 'EC_SANITIZE_SKIP'
  | 'EC_PAYLOADS_DROPPED'
  ...
  | 'EC_FIELD_ENCRYPTION_KEY_MISSING'
  | 'EC_FIELD_ENCODE_FAILED'
  | 'EC_FIELD_ENCRYPT_FAILED'
  | 'EC_FIELD_SPOOL_FAILED'
  | 'EC_WEBHOOK_QUEUE_OVERFLOW'
  | 'EC_LOCAL_EVENT_STORE_WRITE_FAILED'
  | 'EC_WEBHOOK_BATCH_FAILED'
  | 'EC_NEXT_HMR_CLEANUP_FAILED'
  | 'EC_ADAPTIVE_CAPTURE_OVERRIDES_MODE'
  | 'EC_ADAPTIVE_CAPTURE_SWITCH_RATE_LIMITED'
...
export interface SDKConfig {
  ...
  scrubberPolicy?: Partial<Policy>;
  serialization?: Partial<SerializationLimits>;
  maxLocalsCollectionsPerSecond?: number;
  maxCachedLocals?: number;
  maxLocalsFrames?: number;
  /**
   * Adaptive guard for local-variable capture. While locals capture is
   * armed, the V8 debugger pauses on every thrown exception (including
   * caught ones); on exception-heavy hot paths those pauses add up. When
   * pause load exceeds the budget, the SDK temporarily disables locals
   * and annotates packages with `locals: disabled_adaptive_guard`. Locals
   * re-arm after adaptive escalation or a quiet recovery window, unless
   * this is set to 'off' to keep locals armed regardless of pause load.
   */
  localsGuard?: 'off' | { maxPausesPerSecond?: number; maxPauseMsPerMinute?: number };
...
export interface ResolvedConfig {
  ...
  /**
   * Mode-relevant user overrides captured without transport/encryption
   * secrets. Runtime mode switches reapply this subset to every ModeState.
   */
...
export type PackageAssemblyWorkerScrubberPolicy = Omit<Policy, 'piiDetectors'> & {
  piiDetectors?: undefined;
};

export interface PackageAssemblyWorkerConfig extends Omit<ResolvedConfig, 'piiScrubber' | 'scrubberPolicy'> {
  piiScrubber: undefined;
  scrubberPolicy: PackageAssemblyWorkerScrubberPolicy;
}
