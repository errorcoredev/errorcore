...
function createWorkerConfig(config: ResolvedConfig): PackageAssemblyWorkerConfig {
  return {
    ...
    scrubberPolicy: {
      credentialNames: config.scrubberPolicy.credentialNames,
      maxKeys: config.scrubberPolicy.maxKeys,
      spoolBytes: config.scrubberPolicy.spoolBytes,
      maxField: config.scrubberPolicy.maxField
    }
  };
}
...
export class PackageAssemblyDispatcher {
  ...
  /**
   * Refreshes mode-derived package limits without replacing the worker.
   * Worker-port messages are FIFO: assemblies admitted before this call use
   * the previous config, while assemblies posted after it use the new config.
   */
  public updateConfig(
    config: ResolvedConfig,
    options?: { timeoutMs?: number }
  ): Promise<void> {
    const workerConfig = createWorkerConfig(config);
    this.workerData.config = workerConfig;

    if (this.worker === null || !this.available || this.shuttingDown) {
    ...
    return new Promise<void>((resolve, reject) => {
      const timeout = setTimeout(() => {
        this.pending.delete(id);
        reject(new Error('Package assembly worker config update timed out'));
      }, timeoutMs);
      timeout.unref();
      ...
      try {
        this.worker?.postMessage({ id, type: 'update_config', config: workerConfig });
      } catch (error) {
