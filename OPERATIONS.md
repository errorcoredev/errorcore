...
TransportDispatcher sends the serialized payload:
  ...
  - Webhook: append to the local NDJSON event store, then POST batches
...
- Writes every serialized payload to `transport.storePath` before network delivery.
- Sends POST batches to `transport.url` with body `{ version, kind: "errorcore.webhook_batch", sentAt, events }`.
- `events[].payload` is the post-scrubber serialized envelope parsed back to JSON when possible.
- Optional HMAC authentication uses `X-Errorcore-Webhook-Signature: sha256=<hex>` over the exact UTF-8 body and `X-Errorcore-Webhook-Timestamp`.
- Defaults: `batchSize: 100`, `maxDelayMs: 10000`, `retries: 5`, `timeoutMs: 5000`, `maxBufferEvents: 1000`, `retainOnAck: true`.
- When `retainOnAck: false`, acknowledged local records are removed from the event store under the existing file lock. Failed batches remain in the event store.
...
- Keeps up to `maxBackups` rotated copies.
- Event files are created with `0o600` permissions where the platform supports POSIX modes.
...
1. Generate a new key. `node -e "process.stdout.write(require('crypto').randomBytes(32).toString('hex') + '\n')"`.
...
For a terminal snapshot of the newest event, run `npx errorcore show --latest`.
It uses the same NDJSON reader and decrypts when the config supplies
`encryptionKey`.
