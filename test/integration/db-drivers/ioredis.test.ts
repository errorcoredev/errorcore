...
const useExternalRedis = envFlag('EC_INTEGRATION_REDIS');
...
describe.skipIf(Redis === null)('ioredis driver integration', () => {
  ...
  beforeEach(async () => {
    ...
    connectionHost = stub === undefined ? redisHost : '127.0.0.1';
    connectionPort = stub?.port ?? redisPort;
  ...
  it('records a basic SET/GET as outbound db-query events', async () => {
    ...
    expect(setEvent!.target).toBe(`redis://${connectionHost}:${connectionPort}`);
