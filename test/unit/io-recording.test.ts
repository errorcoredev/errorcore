...
describe('Module 08 recorders', () => {
  ...
  it('leaves a third-party emit wrapper in place while hidden SDK wrappers stay inert across re-enable', () => {
    const config = createConfig();
    const buffer = makeBuffer({ capacity: 10, maxBytes: 100000 });
    const als = new ALSManager();
    const tracker = new RequestTracker({ maxConcurrent: 10, ttlMs: 60000 });
    ...
    const originalEmit = Server.prototype.emit;
    const runWithContextSpy = vi.spyOn(als, 'runWithContext');
    const server = new Server();
    ...
    try {
      ...
      server.emit('request', request as unknown as IncomingMessage, new MockServerResponse() as unknown as ServerResponse);
      expect(runWithContextSpy).not.toHaveBeenCalled();

      recorder.install();
      server.emit('request', request as unknown as IncomingMessage, new MockServerResponse() as unknown as ServerResponse);
      expect(runWithContextSpy).toHaveBeenCalledTimes(1);
  ...
  it('does not let an early response-created diagnostic block final cleanup', () => {
    const config = createConfig();
    const buffer = makeBuffer({ capacity: 10, maxBytes: 100000 });
    const als = new ALSManager();
    const tracker = new RequestTracker({ maxConcurrent: 10, ttlMs: 60000 });
    ...
    try {
      ...
      const [context] = tracker.getAll();
      expect(context?.ioEvents.length).toBeGreaterThan(0);

      res.end('ok');

      expect(buffer.drain()).toHaveLength(0);
      expect(context?.ioEvents).toHaveLength(1);
      expect(context?.ioEvents[0]?.phase).toBe('done');
      expect(tracker.getCount()).toBe(0);
    } finally {
      recorder.shutdown();
      tracker.shutdown();
  ...
  it('keeps the original request context when Fastify middleware runs inside a recorder-managed request', async () => {
    ...
    try {
      ...
      expect(sdk.buffer.drain()).toHaveLength(0);
