...
describe('ALSManager', () => {
  ...
  it('copies request metadata while reusing the prepared headers object', () => {
    const manager = new ALSManager();
    const headers = {
      host: 'localhost',
      'x-request-id': 'req-1'
  ...
  it('filters headers lazily once and reflects mutations before materialization', () => {
    const filterHeaders = vi.fn((headers: Record<string, unknown>) => ({
      host: String(headers.host),
      'x-request-id': String(headers['x-request-id'])
    }));
    const manager = new ALSManager({ headerFilter: filterHeaders } as never);
    const headers: Record<string, unknown> = {
      host: 'service.local',
      authorization: 'secret',
      'x-request-id': 'initial'
    };
    ...
    headers.host = 'mutated.local';
    headers['x-request-id'] = 'mutated';

    expect(filterHeaders).not.toHaveBeenCalled();
    expect(context.headers).toEqual({
      host: 'mutated.local',
      'x-request-id': 'mutated'
    });
    expect(context.headers).toBe(context.headers);
    expect(filterHeaders).toHaveBeenCalledTimes(1);
  });

  it('allocates request-local arrays lazily and preserves mutation semantics', () => {
    const manager = new ALSManager();
    const context = manager.createRequestContext({
      method: 'GET',
      url: '/arrays',
    ...
    expect(Object.prototype.hasOwnProperty.call(context, 'ioEvents')).toBe(false);
    expect(Object.prototype.hasOwnProperty.call(context, 'stateReads')).toBe(false);
    expect(Object.prototype.hasOwnProperty.call(context, 'stateWrites')).toBe(false);

    context.ioEvents.push({ requestId: 'req-1' } as never);
    ...
    expect(context.ioEvents).toHaveLength(1);
    expect(context.stateReads).toHaveLength(1);
    expect(context.stateWrites).toHaveLength(1);
  });

  it('generates fixed-width lowercase hex trace and span ids across entropy pool refills', () => {
    const manager = new ALSManager();
    ...
    expect(spanIds.size).toBe(300);
  });

  it('does not reuse released contexts across ALS lifetimes', () => {
    const manager = new ALSManager();
