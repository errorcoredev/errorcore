...
describe('StateTracker', () => {
  ...
  it('merges inbound tracestate before stamping state read sequences', () => {
    const eventClock = new EventClock();
    const als = new ALSManager({
      eventClock,
      config: { traceContext: { vendorKey: 'ec' } }
    });
    const tracker = new StateTracker({ als, eventClock });
    const tracked = tracker.track('cache', new Map([['key', 'value']]));
    const context = als.createRequestContext({
      method: 'GET',
      url: '/state',
      headers: {},
    ...
    als.runWithContext(context, () => tracked.get('key'));

    expect(context.stateReads[0]?.seq).toBeGreaterThan(100);
  });
