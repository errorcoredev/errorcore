...
describe('ALSManager — tracestate ingest and egress (module 21)', () => {
  ...
  it('merges peer clk: into the local EventClock on first trace materialization', () => {
    const eventClock = new EventClock();
    const als = new ALSManager({
      eventClock,
      config: { traceContext: { vendorKey: 'ec' } }
    ...
    expect(eventClock.current()).toBe(1);
    void ctx.traceId;
    // After merge(100), value -> max(1, 100) + 1 = 101.
    expect(eventClock.current()).toBe(101);
  });

  it('defers peer clk merge until trace materialization', () => {
    const eventClock = new EventClock();
    const als = new ALSManager({
      eventClock,
      config: { traceContext: { vendorKey: 'ec' } }
    ...
    expect(eventClock.current()).toBe(0);
    const emitted = als.runWithContext(ctx, () => als.formatOutboundTracestate());
    expect(eventClock.current()).toBe(101);
    expect(emitted).toBe('ec=clk:101,foreign=x');
    expect(als.runWithContext(ctx, () => als.formatOutboundTracestate())).toBe(
      'ec=clk:101,foreign=x'
    );
  });
  ...
  it('chains across two simulated services: B sees seq strictly greater than A', () => {
    ...
    const ctxB = alsB.createRequestContext({
      method: 'GET',
      url: '/b',
      headers: {},
    ...
    alsB.runWithContext(ctxB, () => alsB.formatTraceparent());
