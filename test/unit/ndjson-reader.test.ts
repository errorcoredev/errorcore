...
describe('NdjsonReader payload blob indexing', () => {
  ...
  it('unwraps local event-store and dead-letter records', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'errorcore-ndjson-'));
    tmpDirs.push(dir);
    const filePath = path.join(dir, 'captures.ndjson');
      ...
      [
        {
          ...
          payload: JSON.stringify({
            ...
            capturedAt: '2026-05-06T00:00:00.000Z',
            error: { type: 'Error', message: 'from local store', stack: '' }
          })
        },
        {
          version: 1,
          kind: 'payload',
          storedAt: '2026-05-06T00:01:00.000Z',
          payload: JSON.stringify({
            schemaVersion: '1.2.0',
            eventId: 'evt-dlq',
            fingerprint: 'fp-dlq',
            capturedAt: '2026-05-06T00:01:00.000Z',
            error: { type: 'TypeError', message: 'from dlq', stack: '' }
          }),
          mac: 'not-verified-by-dashboard-reader'
        }
      ].map((entry) => JSON.stringify(entry)).join('\n') + '\n'
    );
    ...
    expect(reader.getAll({ fingerprint: 'fp-local' })).toMatchObject({
      total: 1,
      entries: [expect.objectContaining({ errorMessage: 'from local store' })]
    });
    expect(reader.getAll({ fingerprint: 'fp-dlq' })).toMatchObject({
      total: 1,
      entries: [expect.objectContaining({ errorMessage: 'from dlq' })]
    });
  });
});
