...
import { WebhookTransport } from '../../src/transport/webhook-transport';
...
function createCapturingRequest(statuses: number[]) {
  const bodies: string[] = [];
  ...
  const requestSpy = vi.fn((requestOptions: unknown, callback: (response: EventEmitter) => void) => {
    ...
    request.end = vi.fn(() => {
      ...
      response.statusCode = statuses[Math.min(callCount, statuses.length - 1)];
      response.headers = {};
      callCount += 1;
      callback(response);
      response.emit('data', Buffer.from('ok'));
  ...
  return { requestSpy, bodies, options };
}
...
describe('WebhookTransport', () => {
  ...
  function storePath(): string {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'errorcore-webhook-'));
    dirs.push(dir);
    return path.join(dir, 'events.ndjson');
  }

  it('batches events and signs the exact request body', async () => {
    const captured = createCapturingRequest([200]);
    httpModule.request = captured.requestSpy as typeof httpModule.request;
    ...
    await transport.send({ serialized: JSON.stringify({ eventId: 'evt-1' }) });
    await transport.send({
      kind: 'payload_blob',
      serialized: JSON.stringify({ eventId: 'evt-1', kind: 'payload_blob' })
    });
    await transport.flush();
    await transport.shutdown();

    expect(captured.requestSpy).toHaveBeenCalledTimes(1);
    const body = captured.bodies[0]!;
    const batch = JSON.parse(body) as { events: Array<{ kind: string; payload: unknown }> };
    expect(batch).toMatchObject({
      version: 1,
      kind: 'errorcore.webhook_batch',
      events: [
        { kind: 'error', payload: { eventId: 'evt-1' } },
        { kind: 'payload_blob', payload: { eventId: 'evt-1', kind: 'payload_blob' } }
      ]
    });
    const headers = (captured.options[0] as { headers: Record<string, string> }).headers;
    expect(headers['X-Errorcore-Webhook-Signature']).toBe(
      'sha256=' + createHmac('sha256', 'secret').update(body).digest('hex')
    );
    expect(headers['X-Errorcore-Webhook-Timestamp']).toMatch(/^\d+$/);
  });

  it('retries retryable webhook failures before acknowledging the batch', async () => {
    const captured = createCapturingRequest([500, 200]);
    httpModule.request = captured.requestSpy as typeof httpModule.request;
    ...
    await transport.send({ serialized: '{"eventId":"evt-retry"}' });
    await transport.flush();
    await transport.shutdown();

    expect(captured.requestSpy).toHaveBeenCalledTimes(2);
    randomSpy.mockRestore();
  });

  it('warns and drops the oldest in-memory queued event when the buffer is full', async () => {
    ...
    await transport.send({ serialized: '{"eventId":"old"}' });
    await transport.send({ serialized: '{"eventId":"new"}' });
    await transport.shutdown();

    expect(warnings).toContain('EC_WEBHOOK_QUEUE_OVERFLOW');
  });
});
...
describe('HttpTransport', () => {
  ...
  it('(d) retries exactly 3 times with bounded jittered backoff before throwing', async () => {
    ...
    expect(requestSpy).toHaveBeenCalledTimes(3);
    expect(capturedDelays).toEqual([220, 660]);
...
describe('FileTransport', () => {
  ...
  it('rejects async send when the target path cannot be created', async () => {
    const parentAsFile = path.join(os.tmpdir(), `errorcore-parent-file-${Date.now()}`);
    fs.writeFileSync(parentAsFile, 'not a directory');
    const filePath = path.join(parentAsFile, 'nested', 'errorcore.log');
    const transport = new FileTransport({ path: filePath });
    const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => undefined);
    ...
    try {
      ...
      fs.rmSync(parentAsFile, { force: true });
    }
  });
});
...
describe('TransportDispatcher', () => {
  ...
  it('rejects send when main-thread fallback file transport fails', async () => {
    const parentAsFile = path.join(os.tmpdir(), `errorcore-main-thread-parent-${Date.now()}`);
    fs.writeFileSync(parentAsFile, 'not a directory');
    const filePath = path.join(parentAsFile, 'nested', 'errorcore.log');
    const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => undefined);
  ...
  it('real worker file transport creates missing parent directories', async () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'errorcore-worker-file-dir-'));
