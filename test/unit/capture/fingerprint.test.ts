...
describe('computeFingerprint', () => {
  ...
  it('fingerprints normalized primitive and plain-object throws without throwing', () => {
    const config = resolveTestConfig();
    const values = ['oops', 42, null, undefined, { kind: 'plain-object' }];

    for (const value of values) {
      const normalized = normalizeThrown(value, config);

      expect(normalized).toBeInstanceOf(Error);
      expect(computeFingerprint(normalized, [])).toMatch(/^[0-9a-f]{16}$/);
    }
  });
});
