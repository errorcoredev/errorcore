...
describe('UI dashboard server', () => {
  ...
  it('filters the error list by fingerprint', async () => {
    const filePath = freshFile();
    paths.push(filePath);
    fs.writeFileSync(
      filePath,
      [
        {
          ...
          error: { type: 'Error', message: 'first', stack: '' }
        },
        {
          schemaVersion: '1.2.0',
          eventId: 'evt-b',
          ...
          error: { type: 'Error', message: 'second', stack: '' }
        }
    ...
    const response = await fetch(`http://127.0.0.1:${port}/api/errors?fingerprint=fp-b`);
    expect(response.status).toBe(200);
    const body = await response.json() as { total: number; entries: Array<{ errorMessage: string }> };
    expect(body).toEqual({
      total: 1,
      entries: [expect.objectContaining({ errorMessage: 'second' })]
    });
  });
});
