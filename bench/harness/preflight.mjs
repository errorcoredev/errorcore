...
export const CANDIDATE_VERSION = readJson(path.join(repoRoot, 'package.json')).version;
...
function hasExactSha256Digest(value) {
...
export function validateBenchmarkManifest(manifest) {
  ...
  if (manifest?.errorcore?.version !== CANDIDATE_VERSION) {
    errors.push(`errorcore version must match root package ${CANDIDATE_VERSION}`);
  }
...
export async function collectPreflightFacts(options = {}) {
  ...
  const facts = applyHostToolCache({
    ...
    candidateSha256: process.env.BENCH_CANDIDATE_SHA256 || undefined,
    diagnosticOverrides: {
      middleware: process.env.BENCH_ERRORCORE_MIDDLEWARE || undefined,
      locals: process.env.BENCH_ERRORCORE_LOCALS || undefined
    },
...
export function validatePreflightFacts(facts, manifest) {
  ...
  if (facts.candidateSha256 !== undefined && !hasExactSha256Digest(facts.candidateSha256)) {
    errors.push('BENCH_CANDIDATE_SHA256 must be sha256 followed by exactly 64 hexadecimal characters');
  }
  if (facts.diagnosticOverrides?.middleware !== undefined) {
    errors.push(
      'BENCH_ERRORCORE_MIDDLEWARE is diagnostic-only and cannot be used for run-all scoring; ' +
      'use harness/run-perf-only.mjs'
    );
  }
  if (facts.diagnosticOverrides?.locals !== undefined) {
    errors.push(
      'BENCH_ERRORCORE_LOCALS is diagnostic-only and cannot be used for run-all scoring; ' +
      'use harness/run-perf-only.mjs'
    );
  }
