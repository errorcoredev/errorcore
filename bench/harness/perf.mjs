...
async function runAutocannon(url, options = {}) {
  ...
  const bin = path.join(appRoot, 'node_modules', 'autocannon', 'autocannon.js');
  const result = await runCommand(process.execPath, [bin, '-j', '-c', connections, '-d', duration, url], { cwd: appRoot, env: process.env });
  if (result.code !== 0) {
    return {
      skipped: true,
