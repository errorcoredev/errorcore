...
function releaseGateText(releaseGate) {
  if (releaseGate?.enabled !== true) {
    return 'Strict release gate was not enabled for this exploratory run.';
  }
  if (releaseGate.ok === true) {
    return `Strict release gate passed for ${releaseGate.captureMode ?? 'unknown'} mode.`;
  }
  const errors = (releaseGate.errors ?? []).map((error) =>
    `- ${error.code ?? 'RELEASE_GATE_ERROR'}: ${error.message ?? String(error)}`
  );
  return [
    `Strict release gate failed for ${releaseGate.captureMode ?? 'unknown'} mode with ${errors.length} error(s).`,
    '',
    ...(errors.length > 0 ? errors : ['- RELEASE_GATE_ERROR: no failure detail was recorded'])
  ].join('\n');
}
...
export function writeReport({
  ...
  releaseGate = { enabled: false, ok: true, errors: [] },
  promptPath,
  artifactPaths = {}
}) {
  ...
  const body = [
    ...
    `Candidate tarball SHA-256: ${preflight.facts.candidateSha256 ?? ''}`,
