...
import { getScenarioMatrix, requestForScenario, expectedPayloadCount, getSdkVariants } from './scenarios.mjs';
import { requestJson, waitFor } from './http.mjs';
import { spawnBenchmarkApp } from './process-runner.mjs';
import { perfBySdkForScoring, runPerfSuite } from './perf.mjs';
...
async function runVariant({ scenario, sdk, basePort }) {
  ...
  try {
    ...
    const events = await waitFor(`${sdk} ${scenario.id} payloads`, async () => {
      const current = await fetchSinkEvents(sdk, scenario.id);
      return current.length >= expected ? current : false;
    }, { timeoutMs: scenario.id === 'S9' ? 45_000 : 15_000, intervalMs: 500 }).catch(async () => {
      return fetchSinkEvents(sdk, scenario.id);
    });
...
async function runScenarios() {
  ...
  const matrix = getScenarioMatrix();
...
const perfResults = await runPerfSuite({
  resultsDir,
...
}).catch((error) => [
  {
    ...
    repetitions: [],
    aggregates: [
      { sdk: 'all', endpoint: 'all', skipped: true, reason: error instanceof Error ? error.message : String(error) }
    ],
    sanity: []
  }
][0]);
