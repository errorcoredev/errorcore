...
export function createScenarioEngine(options) {
  ...
  async function runScenario(scenarioId, requestContext = {}) {
      ...
      setTimeout(() => {
        void (async function delayedUpstreamFailureAfterResponse() {
          try {
            ...
            await capture(captured, { scenarioId, phase: 'delayed', request: delayedRequest });
          }
        })();
      }, 25).unref();
      return json(202, { accepted: true, scenarioId });
    ...
    if (scenarioId === 'S7') {
      ...
      await dependencies.pushJob(queue, {
        jobType: 'reprice',
        payload: {
          malformed: true,
          price: { cents: 'not-an-integer' },
        },
      });
      const queueRequest = {
        method: requestContext.method,
      ...
      setTimeout(() => {
        void (async function queueConsumer() {
          try {
            ...
            await capture(decorate(error, scenarioId, options), {
              scenarioId,
              phase: 'queue-consumer',
