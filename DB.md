...
## Local event store format

Webhook transport uses a local NDJSON event store before shipping batches. Each line wraps the exact serialized payload so the dashboard can read the same file and failed webhook delivery never loses the local copy.
...
```json
{
  ...
  "payloadKind": "error",
  "payload": "<serialized Errorcore envelope as string>"
}
```

`payloadKind` is `error` or `payload_blob`. When webhook `retainOnAck` is false, acknowledged records are removed by rewriting the file under the file lock.
